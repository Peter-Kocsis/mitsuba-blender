...
def convert_emitter_materials_cycles(export_ctx, current_node):
    ...
    if current_node.inputs['Color'].is_linked:
        color_node = current_node.inputs['Color'].links[0].from_node
        if current_node.inputs['Strength'].is_linked:
            raise NotImplementedError("Only default strength is supported.")  # TODO: rgb input
        else:
            strength = current_node.inputs['Strength'].default_value

        if color_node.name == 'Blackbody':
            # Blackbody is supported only in spectral mode
            # -> Approximate it by single color emission
            # TODO: Consider the strength!
            temperature = color_node.inputs['Temperature'].default_value
            radiance = [float(intensity) / 255 * strength for intensity in convert_K_to_RGB(temperature)]
            params_radiance = export_ctx.spectrum(radiance)
